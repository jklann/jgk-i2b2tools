...
data_dir <- '/4ceData_x2/4ceData_x2/101321/'
...
ze  <- obs_raw %>% group_by(concept_code) %>% summarise(n=n()) %>% filter(n>1000)
...
iterate_associations_loopy = function(assoc_input_df,feature_list,size_of_combos,set_size) {
  starttime <- Sys.time()
  i <- 0
...
association_loop <- function(input_df,max_set_size,logical_and,min_support,min_sens,min_spec,min_ppv,features) {
  if(missing(features)) {
  # Use the super-fast set version to select initial features (it doesn't output in the right format)
    quickout <- iterate_associations(input_df)
  ...
  } else { feature_list_short <- features }
  output_df <- list()
  
  print(glue("Prepared for loop with {length(feature_list_short$concept_code)} features."))
  for(i in 1:max_set_size) {
    outout <- iterate_associations_loopy(assoc_input_df,feature_list_short$concept_code,i,ifelse(logical_and,i,1)) %>% filter(sens>min_sens,spec>min_spec,ppv>min_ppv,supp>min_support)
    ...
    print(glue("Finished round {i} with {length(feature_list_short$concept_code)} remaining features..."))
  }
  return(output_df)
}
...
```{r}
baseline <- iterate_associations_loopy(assoc_input_df,c("covidpos"),1,1)
master_output_or<-bind_rows(association_loop(assoc_input_df,3,FALSE,0.1,0.4,baseline$spec,0.1),.id="round")

association_loop(assoc_input_df,3,FALSE,0,0,0,0,feature_list %>% filter(concept_code %in% c("48065-7","48066-5","2276-4","1988-5")))
...
master_output_or<-bind_rows(association_loop(assoc_input_df,2,FALSE,0.1,0.1,0.1,0.1),.id="round") # ORs of size <=2
#master_output<-bind_rows(association_loop(assoc_input_df,4,TRUE,0.1,0.4,0.2,0.1),.id="round") # ANDs of size <=4
...
master_output_and<-bind_rows(association_loop(assoc_input_df,4,TRUE,0.1,0.4,0.2,0.1),.id="round")
...
save(list = site_results, file = file.path("/4ceData/out", paste0(currSiteId, "_dataminingresults.rda")))
```
