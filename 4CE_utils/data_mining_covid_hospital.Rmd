...
* Calculate using three time intervals (1st wave, slump, 2nd wave)
* auto-pick best points? 
* Speed optimize when calculating suppot of full dataset
...
To run this code:
1) Set your site ID, optionally data directory, and output directory.
...
5) Click the "knit" button in RStudio. This could take 20 minutes.
6) Please share the output html and optionally the "[site]_dataminingresults.rda" file in the output directory.
...
total_ct_full <- assoc_input_df_full %>% select(c(patient_num)) %>% distinct() %>% nrow() %>% first()
...
feature_list <- assoc_input_df %>% select(concept_code) %>% distinct_all()
...
association_loop <- function(input_df,input_df_full,max_set_size,logical_and,min_support,min_sens,min_spec,min_ppv,features) {
  ...
  # Uses the FULL dataset to compute support
    ...
    print(typeof(feature_list_short))
  } else { feature_list_short <- features; feature_list_groups <- features }
  output_df <- list()
  
  print(glue("Prepared for loop with {length(feature_list_short$concept_code)} features."))
  for(i in 1:max_set_size) {
    outout <- iterate_associations_loopy(assoc_input_df,feature_list_short$concept_code,i,ifelse(logical_and,i,1),assoc_input_df_full,feature_list_groups) %>% filter(sens>min_sens,spec>min_spec,ppv>min_ppv,supp_full>min_support)
    ...
    print(glue("Finished round {i} with {length(feature_list_short$concept_code)} remaining features and {length(feature_list_groups)} combinations to add to..."))
  }
  return(output_df)
}
...
for(q in quarters){
  ...
  master_output_or_s<-bind_rows(association_loop(q_assindf,q_assindf_full,1,FALSE,0.2,0.2,0.5,0.5),.id="round") # ORs of size <=2
  out_df<-append(out_df,c(list(master_output_or_s)))
  
  fig <- ggplot(master_output_or_s,aes(color=round,text=paste("plot#:",i_count,"\nPPV:",ppv,"\n",combo_eng,"\n",combo,"\nSupport:",supp),x=round(spec,2),y=round(sens,2))) + geom_point(position=position_jitter(h=0,w=0.1)) +
  #scale_x_discrete(labels=c()) + scale_y_discrete(labels=c()) +
  labs(title="Sensitivity vs. Specificity of Proxies for COVID-Admission", 
       x = "Specificity", y="Sensitivity",fill="Size of Predictive Set") +
  scale_x_reverse() + 
...
for(q in quarters){
  ...
  q_assindf <- assoc_input_df %>% filter(c(as.character(cohort))==q[1] | c(as.character(cohort))==q[2])
  q_assindf_full <- assoc_input_df_full %>% filter(c(as.character(cohort))==q[1] | c(as.character(cohort))==q[2])
  master_output_and_s<-bind_rows(association_loop(assoc_input_df,assoc_input_df_full,2,TRUE,0.3,0.4,0.2,0.2),.id="round")
  out_df_and<-append(out_df,c(list(master_output_and_s)))
  
  fig <- ggplot(master_output_and_s,aes(color=round,text=paste("plot#:",i_count,"\nPPV:",ppv,"\n",combo_eng,"\n",combo,"\nSupport:",supp),x=round(spec,2),y=round(sens,2))) + geom_point(position=position_jitter(h=0,w=0.1)) +
  #scale_x_discrete(labels=c()) + scale_y_discrete(labels=c()) +
  labs(title="Sensitivity vs. Specificity of Proxies for COVID-Admission", 
       x = "Specificity", y="Sensitivity",fill="Size of Predictive Set") +
  scale_x_reverse() + 
  scale_color_brewer(palette="RdYlGn") 
  out_fig_and<-append(out_fig,c(list(fig)))
}

subplot(out_fig_and[[1]],out_fig_and[[2]],out_fig_and[[3]],out_fig_and[[4]],nrows=2,margin=0.04) %>% style(showlegend=FALSE, traces = 2:5)
```
