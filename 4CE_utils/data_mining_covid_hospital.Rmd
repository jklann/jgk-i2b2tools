...
iterate_associations_loopy = function(assoc_input_df,feature_list,size_of_combos,set_size) {
  ...
  for(x in combinations) {
    ...
    l_patient_set <- reduced_input_df %>% filter(concept_code %in% x) %>% group_by(patient_num) %>% summarise(howmany=n()) %>% filter(howmany==set_size) %>% select (patient_num) # 1[?]
...
association_loop <- function(input_df,max_set_size,logical_and,min_support,min_sens,min_spec,min_ppv) {
...
master_output<-bind_rows(association_loop(assoc_input_df,5,TRUE,0.1,0.5,0.2,0.1),.id="round")

fig <- ggplot(master_output,aes(color=round,text=paste("PPV:",ppv,"\n",combo_eng,"\n",combo,"\nSupport:",supp),x=round(spec,2),y=round(sens,2))) + geom_point() +
