...
4) Configure your data directory below if it is not the one specified in the 4CE package (for example, if using 2.2 data - which incidentally seems to work fine with this script)
...
if (fake_chart_review) { # fake number
  chart_raw <- tibble(
    patient_num = as.character(demo_raw$patient_num[1:50]),
...
pt_level_df <- obs_raw %>%
  ...
  filter(row_number() == 1) %>% # jgk 9-2-21, maybe helpful: variable # rows per patient seemed concerning
  ungroup()

pre_pts_byweek <- pt_level_df %>%
  select(label, patient_num, admission_date, admitted_for_covid) %>%
  distinct() %>%
  group_by(week = week(admission_date) + ((year(admission_date) - 2020) * 52), label) %>%
  summarise(
    truedot = sum(admitted_for_covid, na.rm = TRUE), # jgk 9-2-21, handle maybes coded as 2
...
n1 <- pts_byweek %>%
  group_by(label) %>%
  summarise(sum_true = sum(truedot), sum_false = sum(falsedot), .groups = "drop")
...
pre_pts_byweek_nofilter <- pt_level_df %>%
  select(label, patient_num, admission_date, admitted_for_covid) %>%
  distinct() %>%
  group_by(week = week(admission_date) + ((year(admission_date) - 2020) * 52)) %>%
  summarise(
    truedot = sum(admitted_for_covid, na.rm = TRUE), # jgk 9-2-21, handle maybes coded as 2
    falsedot = sum(!admitted_for_covid, na.rm = TRUE),
    n_week = n() %>% na_if(0), .groups = "drop"
...
pct_chart_admit <- pre_pts_byweek_nofilter %>%
  mutate(pct = (truedot / (truedot + falsedot) * 100)) %>%
  drop_na() %>%
  transmute(week, n_week = pct, week_name = ymd("2020-01-01") + weeks(week))

# Plot % of chart-reviewed patients admitted for COVID, by week
ggplot(data = pct_chart_admit, aes(y = n_week, x = week_name)) +
  geom_point(label = "# Chart-reviewed") +
  # depending on the data, geom_smooth may not make sense here
  # geom_smooth(method = "loess") +
  labs(
    y = "Percent found to be admitted for COVID",
    x = "Week (starting Jan 2020)",
    title = "Percent of chart-reviewed 4CE patients actually admitted for COVID, by week"
  ) +
  theme_classic() +
