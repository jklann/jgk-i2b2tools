...
fn_summarize_byweek <- function(df, dflabel) {
  mydf <- df %>%
    ...
    group_by(week) %>%
    summarise(n_week = sum(n), .groups = "drop") %>%
    mutate(
      label = dflabel,
      n_week = na_if(n_week, 0)
    )
...
pct_pts <- all_pts_byweek %>%
  inner_join(magic_pts_byweek, by = "week") %>%
  replace_na(list(n_week.x = 0, n_week.y = 0)) %>%
  transmute(week, n_week = 100 - (n_week.y / n_week.x * 100))
#
...
diff_pts_byweek <- all_pts_byweek %>%
  left_join(magic_pts_byweek, by = "week") %>%
  transmute(week, n_week = n_week.x - n_week.y) %>%
  mutate(label = "diff")
...
covid_tn <- nocovid_pts_byweek %>%
  left_join(covid_fp, by = "week") %>%
  replace_na(list(n_week.x = 0, n_week.y = 0)) %>%
  transmute(week,
         n_week = n_week.x - n_week.y %>% na_if(0),
         label = "covid_TN") 

# False negatives (all positive - true positive)
covid_fn <- covid_pts_byweek %>%
...
# Union, summarise
# pts_byweek <- all_pts_byweek %>%
#   union_all(diff_pts_byweek_size) %>%
#   union_all(magic_pts_byweek_size)
pts_byweek <- all_pts_byweek %>% 
  bind_rows(diff_pts_byweek_size, magic_pts_byweek_size) %>%
  mutate(
    truedottype = ifelse(is.na(truedot), NA, "TRUE"),
    falsedottype = ifelse(is.na(falsedot), NA, "FALSE"),
...
plot_filter <- function(df, label = "") {
  ...
  # Build annotation
  grob <- grid::grobTree(grid::textGrob(paste("Sens:", trunc(summary_stats[1] * 100), ",Spec:", trunc(summary_stats[2] * 100)),
  ...
  ggplot() +
    ...
    labs(
      y = "Number of admissions by week", x = "Week (starting Jan 2020)",
...
```{r}
polished_pts_byweek <- pts_byweek %>% 
  mutate(label = label %>% fct_recode(
    "All patients" = "all",
    "Kept patients" = "selected",
    "Removed patients" = "diff"
  )) 

rug_df <- polished_pts_byweek %>%
...
ggplot(aes(y = n_week, x = week_name)) +
  ...
  scale_x_date(expand = expansion(c(0, 0), c(10, 60))) +
  ggtitle(paste("Patients removed by non-COVID-admission filter\n ")) +
  labs(
    color = NULL,
    y = "Number of admissions by week", x = NULL,
    caption = glue::glue("Site: {currSiteId}
...
polished_pts_byweek %>% 
  ...
  # geom_point(alpha = 0.8, aes(y = n_week, x = week_name, size = falsedot),  shape = 21, stroke = 2, color = '#D95F02') +
  theme_classic() +
  scale_linetype_manual(values = c(3, 2, 1)) +
  scale_alpha_discrete(range = c(0.5, 1)) +
  scale_color_brewer(palette = "Dark2", direction = -1) +
...
# Blur
pts_byweek_blur <- pts_byweek %>%
  blur_it("n_week", blur, threshold) %>%
  select(week, n_week, label, week_name, truedot, falsedot, truedottype, falsedottype)
