...
pt_level_df <- obs_raw %>%
  ...
  group_by(patient_num) %>% arrange(days_since_admission) %>% filter(row_number()==1) # jgk 9-2-21, maybe helpful: variable # rows per patient seemed concerning
...
pct_chart_admit <- pre_pts_byweek_nofilter %>%
  ...
  subset(select=c(week,pct,truedot,falsedot)) %>%
